...
if (isTemplateLoaded instanceof Promise) {
  isTemplateLoaded.then(() => {
    // new callback added on EVERY call
...
### PERF-6 · LACK OF REACTIVE `HASS` TRACKING

**File:** `streamline-card.js`
**Issue:** Cards re-evaluate JS templates for _any_ HA state change, even if the entities used in the template haven't changed.
**Fix:** Implement a Proxy-based Hass Tracker to record entity access during JS evaluation and only re-run when dependencies change.

### PERF-7 · Recursive Config Crawling

**File:** `evaluateJavascript-helper.js`
**Issue:** `evaluateJavascript` recursively walks the entire object tree on every update.
**Fix:** On the first parse, build a path map of all `_javascript` keys to skip the crawl and only update specific paths.

### PERF-8 · String-based Variable Replacement

**File:** `evaluateVariables-helper.js`
**Issue:** Multiple regex `replaceAll` calls on stringified JSON.
**Fix:** Split templates into static chunks and variable indices for array-join evaluation.
...
| BUG-9  | both files                        | Dual independent loaders                    | 🔴 Critical |
| PERF-1 | `evaluateVariables-helper.js:53`  | `JSON.stringify` per hass update × 30 cards | 🔴 Critical |
| PERF-6 | `streamline-card.js`              | Lack of reactive hass tracking              | 🔴 Critical |
...
| PERF-8 | `evaluateVariables-helper.js`     | Slow string-based variables                 | 🟠 High     |
...
| PERF-7 | `evaluateJavascript-helper.js`    | Recursive config crawling                   | 🟡 Medium   |
...
### Phase 3 — Main-thread performance (PERF-1..8)
...
14. Implement Proxy-based Hass Tracker (PERF-6).
15. Implement path map for targeted JS evaluation (PERF-7).
16. Implement pre-compiled templates for variables (PERF-8).

### Phase 4 — Correctness fixes (BUG-4, BUG-7)

17. `streamline-card-editor.js:158` — fix sort comparator with `indexOf` (BUG-4).
18. `evaluateJavascript-helper.js` — make `processConfig` return a new object, not mutate (BUG-7).

### Phase 5 — Cleanup (ARCH-2 through ARCH-6)

19. Remove `_accessedProperties` dead field.
20. Replace `_isConnected` with `this.isConnected`.
21. Rename `evaluteConfig-helper.js` → `evaluateConfig-helper.js`, update all imports.
22. Rename `getLovelace.helper.js` → `getLovelace-helper.js`, update all imports.
23. Replace `new Event + event.detail =` with `new CustomEvent(…, { detail })`.
